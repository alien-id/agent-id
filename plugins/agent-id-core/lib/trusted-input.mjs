...
/**
 * Write a non-secret notice to the controlling terminal (best-effort, no-op if
 * /dev/tty can't be opened). Used to surface context (a title/description) on the
 * SAME surface as promptSecret, so the human sees it even when the process's
 * stdout/stderr are piped to an agent.
 */
export function notifyTty(message) {
  if (!POSIX) return;
  let fd;
  try {
    fd = openTty();
