...
test("BrowserFormProvider.isAvailable() honors AGENT_ID_NO_BROWSER", () => {
  ...
  assert.equal(new BrowserFormProvider({ env: { AGENT_ID_NO_BROWSER: "1" } }).isAvailable(), false);
});
...
test("resolver: an unavailable extraProvider is skipped", () => {
  const p = resolveSecurePrompt({ env: {}, extraProviders: [stub("mobile", { available: false })] });
...
test("resolver: a multiline spec skips providers that can't do multiline", () => {
  ...
  assert.equal(
    resolveSecurePrompt({ env: {}, extraProviders: [single], need: { multiline: true } }).name,
    "browser",
  );
});
...
test("hosted: selected for a valid unix socket; collect() round-trips values; URLs refused", async () => {
  ...
  try {
    ...
    const { values } = await provider.collect({ fields: [{ name: "otp" }], timeoutMs: 5000 });
    ...
    assert.equal(
      resolveSecurePrompt({ env: { AGENT_ID_SECURE_PROMPT_SOCK: "http://127.0.0.1:9/" } }).name,
      "browser",
    );
  } finally {
    server.close();
...
test("BrowserFormProvider.collect delegates to collectViaForm (driven via onUrl, no browser)", async () => {
  ...
  const done = provider.collect({ fields: [{ name: "value" }], open: false, onUrl: resolveUrl });
  done.catch(() => {});
