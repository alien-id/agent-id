...
test(
  "snapshot never surfaces a text field's value, but keeps a submit's label",
  { skip },
  async () => {
    ...
    assert.ok(
      !names.includes("778899"),
      "OTP value must not leak into snapshot names"
    );
    assert.ok(
      !names.includes("alice@example.com"),
      "text-field value must not leak into snapshot names"
    );
    assert.ok(
      !snap.elements.some((e) =>
        /778899|alice@example\.com|hunter2/.test(e.name)
      ),
...
test(
  ...
  async () => {
    ...
    assert.ok(
      !JSON.stringify(snap).includes("alice@example.com"),
...
test(
  "ordinary form fill refuses password fields without echoing the value",
  { skip },
  async () => {
    const snap = await page.evaluate(formSnapshotInPage, {
      prefix: "",
...
test(
  "read-back guard: password refused, benign text allowed, tainted field refused",
  { skip },
  async () => {
    // A password field is refused (the original guard).
    await assert.rejects(
      () => refusePasswordRead(page, "#pw"),
      /password|secret/i
    );
...
test(
  "taint survives a re-snapshot and keeps suppressing the value",
  { skip },
  async () => {
    ...
    const stillTainted = await page.$eval(
      "#otp",
      (el, attr) => el.hasAttribute(attr),
      SECRET_TAINT_ATTR
    );
    assert.equal(
      stillTainted,
      true,
      "re-snapshot must not strip the taint attribute"
    );
    assert.ok(
