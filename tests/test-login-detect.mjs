...
test("logged-in: no password field, no otp, no error", () => {
  assert.equal(
    classifyLogin({ hasPasswordField: false, bodyText: "Welcome back, alice" }),
    "logged-in"
  );
});
...
test("otp-required: a one-time-code input is present", () => {
  assert.equal(
    classifyLogin({ hasPasswordField: false, hasOtpField: true }),
    "otp-required"
  );
});
...
test("otp-required: an OTP-ish field name", () => {
  assert.equal(
    classifyLogin({ hasPasswordField: false, otpFieldNames: ["totp_code"] }),
    "otp-required"
  );
});
...
test("otp-required takes precedence over an error message", () => {
  assert.equal(
    classifyLogin({
      ...
      hasOtpField: true,
      bodyText: "invalid code, try again",
    }),
    "otp-required"
  );
});
...
test("failed: a credential error and no OTP affordance", () => {
  assert.equal(
    classifyLogin({
      hasPasswordField: true,
      bodyText: "That password is incorrect. Try again.",
    }),
    "failed"
  );
});
...
test("unknown: password field present, no error, no OTP", () => {
  assert.equal(
    classifyLogin({ hasPasswordField: true, bodyText: "Sign in to continue" }),
    "unknown"
  );
});
...
test("blocked: takes precedence over a vanished password field (logged-in) and over errors", () => {
  ...
  assert.equal(
    classifyLogin({
      hasPasswordField: false,
      bodyText: "unusual traffic detected",
    }),
    "blocked"
  );
  ...
  assert.equal(
    classifyLogin({
      hasPasswordField: true,
      bodyText: "Verify you are human. try again",
    }),
    "blocked"
  );
});
...
test("blocked: an explicit blocked flag forces the outcome", () => {
  assert.equal(
    classifyLogin({ hasPasswordField: false, blocked: true, bodyText: "" }),
    "blocked"
  );
});
...
test("blocked: a captcha / verification challenge is caught", () => {
  assert.equal(
    classifyLogin({
      hasPasswordField: false,
      bodyText: "Are you a robot? Complete the challenge to continue.",
    }),
    "blocked"
  );
});
...
test("NOT blocked: ordinary app copy that merely contains benign words stays logged-in", () => {
  assert.equal(
    classifyLogin({
      hasPasswordField: false,
      bodyText: "Traffic report: your commute is clear today",
    }),
    "logged-in"
  );
});
...
test("the identifier signal does not outrank a block wall or a credential error", () => {
  ...
  assert.equal(
    classifyLogin({
      hasIdentifierField: true,
      onLoginPage: true,
      errorText: "That email is not recognized",
    }),
    "failed"
  );
});
...
for (const [name, bodyText] of IDENTIFIER_STEPS) {
  test(`identifier step (${name}) is not a finished login`, () => {
    assert.equal(
      classifyLogin({ hasIdentifierField: true, onLoginPage: true, bodyText }),
      "unknown"
    );
  });
}
...
const CODE_STEPS = [
  ...
  [
    "notion",
    {
      otpFieldNames: ["Enter code"],
      bodyText: "Check your inbox\nEnter the login code we just emailed you.",
    },
  ],
  [
    "airbnb sms",
    {
      hasOtpField: true,
      bodyText: "Confirm your number\nEnter the code we sent to +1 ***",
    },
  ],
  [
    "amazon",
    {
      otpFieldNames: ["otpCode"],
      bodyText: "Two-Step Verification\nEnter OTP",
    },
  ],
  [
    "confirmation-code wording",
    { bodyText: "We emailed you a confirmation code. Please enter it below." },
  ],
  [
    "access-code wording",
    { bodyText: "Enter the access code sent to your email" },
  ],
];
...
test("a mailed sign-in LINK is neither a code step nor a finished login", () => {
  ...
  assert.equal(
    classifyLogin({
      bodyText:
        "Click the link in the email we just sent to finish signing in.",
    }),
    "magic-link"
  );
});
...
test("a page offering BOTH a link and a code is an ordinary code step", () => {
  assert.equal(
    classifyLogin({
      hasOtpField: true,
      bodyText:
        "We sent a magic link to you@example.com — or enter the 6-digit code below.",
    }),
    "otp-required"
  );
});
...
for (const [name, bodyText] of SIGNED_IN_PAGES) {
  test(`${name} on a signed-in page stays logged-in`, () => {
    assert.equal(
      classifyLogin({ hasPasswordField: false, bodyText }),
      "logged-in"
    );
  });
}
...
test("a QR that signs you in ELSEWHERE is the linked-devices panel, not a sign-in screen", () => {
  ...
  assert.equal(
    classifyLogin({ bodyText: "Scan the QR code to sign in" }),
    "qr-sign-in"
  );
});
...
test("an unqualified 'enter the code' still reads as a code step", () => {
  ...
  assert.equal(
    classifyLogin({ bodyText: "Enter the code below to continue" }),
    "otp-required"
  );
  assert.equal(
    classifyLogin({
      hasPasswordField: false,
      bodyText: "Enter your promo code at checkout",
    }),
    "logged-in"
  );
});
...
test("a code screen that also mentions a link is a code step, however weak its field", () => {
  ...
  assert.equal(
    classifyLogin({
      ...
      otpFieldNames: [""],
      bodyText:
        "Enter the 6-digit code we sent, or use the login link in the same email",
    }),
    "otp-required"
  );
  assert.equal(
    classifyLogin({
      bodyText:
        "Log in with a QR code, or enter the verification code we sent you",
    }),
    "otp-required"
  );
});
...
test("failed: a Russian rejection is read as a bad credential, not `unknown`", () => {
  ...
  assert.equal(
    classifyLogin({
      hasPasswordField: true,
      bodyText: "Ошибка авторизации. Попробуйте ещё раз.",
    }),
    "failed"
  );
});
...
test("Russian body copy without a rejection does not trip the error match", () => {
  ...
  assert.equal(
    classifyLogin({
      hasPasswordField: false,
      bodyText: "Добро пожаловать, Иван",
    }),
    "logged-in"
  );
});
...
test("codeDestination reads back the destination the page itself printed", () => {
  const cases = [
    ...
    [
      "Check your inbox — we sent a code to your email address.",
      "your email address",
    ],
  ];
...
test("codeDestination also reads the shape that names no `to`", () => {
  ...
  const cases = [
    ...
    [
      "Enter the code we sent to your phone ending in 4817.",
    ...
    [
      "Enter the code sent to your mobile ending with ••4817",
      "your mobile ending with ••4817",
    ],
    [
      "We messaged your mobile ending with ••4817",
      "your mobile ending in ••4817",
    ],
  ];
  for (const [body, expected] of cases) {
    assert.equal(codeDestination(body), expected, body);
  ...
  assert.equal(
    codeDestination(
      "We sent a code to d••@gmail.com — the address ending in 4817 is your old one"
    ),
    "d••@gmail.com"
  );
});
