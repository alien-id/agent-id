...
describe("EIP-1559 signing (cross-validated against eth-account)", () => {
  it("vector 1: simple value transfer on Polygon (chainId 137)", () => {
    ...
    assert.equal(hash, "0x0ee145f3ce3051c0e14fddb1b157c0f6955cba05a348c730c98bee4a4e24c23d");
  ...
  it("requires explicit gas/fee/nonce/chainId fields", () => {
    assert.throws(
      () => signEip1559Transaction({ chainId: 1, to: "0x" + "11".repeat(20) }, TEST_KEY),
      /required/,
    );
  });
});
...
describe("JSON-RPC body transform", () => {
  ...
  it("rewrites eth_sendTransaction → eth_sendRawTransaction (signed)", () => {
    const body = JSON.stringify({ jsonrpc: "2.0", id: 1, method: "eth_sendTransaction", params: [txObj] });
    const { body: out, signed, hashes } = signEvmRpcBody(body, TEST_KEY.toString("hex"), TEST_ADDRESS);
    assert.ok(signed);
    assert.equal(hashes.length, 1);
    const parsed = JSON.parse(out);
    assert.equal(parsed.method, "eth_sendRawTransaction");
    assert.match(parsed.params[0], /^0x02f874/);
    assert.equal(hashes[0], "0x0ee145f3ce3051c0e14fddb1b157c0f6955cba05a348c730c98bee4a4e24c23d");
  });
  ...
  it("passes other methods through untouched", () => {
    const body = JSON.stringify({ jsonrpc: "2.0", id: 9, method: "eth_getTransactionCount", params: [TEST_ADDRESS, "latest"] });
  ...
  it("passes non-JSON bodies through untouched", () => {
    const r = signEvmRpcBody("plain text", TEST_KEY.toString("hex"), TEST_ADDRESS);
    assert.equal(r.signed, false);
