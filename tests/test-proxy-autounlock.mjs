...
const PROXY_ROOT = new URL("../plugins/agent-id-proxy", import.meta.url)
  .pathname;
...
function runHook(dir) {
  return spawnSync("bash", [HOOK], {
    ...
    env: {
      ...process.env,
      AGENT_ID_STATE_DIR: dir,
      CLAUDE_PLUGIN_ROOT: PROXY_ROOT,
...
test("autounlock subcommand writes / removes the marker", async () => {
  ...
  try {
    let r = spawnSync("node", [CLI, "autounlock", "--state-dir", dir], {
      encoding: "utf8",
    });
    assert.equal(r.status, 0, r.stderr);
    await access(path.join(dir, "autounlock")); // exists

    r = spawnSync("node", [CLI, "autounlock", "--off", "--state-dir", dir], {
      encoding: "utf8",
    });
    assert.equal(r.status, 0, r.stderr);
...
test("hook is a no-op without the marker, and without a vault", async () => {
  ...
  try {
    ...
    assert.equal(
      existsSync(path.join(dir, "autounlock.log")),
      false,
      "must not launch without the marker"
    );
    ...
    assert.equal(
      existsSync(path.join(dir, "autounlock.log")),
      false,
      "must not launch without a vault"
    );
  } finally {
    await rm(dir, { recursive: true, force: true });
  }
