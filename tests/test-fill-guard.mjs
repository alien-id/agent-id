...
import { assertFillAllowed, hostOfUrl } from "../plugins/agent-id-browser/lib/session-server.mjs";

const login = (over = {}) => ({ type: "login", domains: ["portail.unige.ch"], ...over });
const sealedWallet = { type: "solana-keypair", exportable: false, domains: ["rpc.x"], secretSeed: "ab".repeat(32) };
...
test("SEAL: a sealed (exportable:false) secret field is refused on any host", () => {
  assert.throws(() => assertFillAllowed(sealedWallet, "rpc.x", "secretSeed"), /sealed/i);
...
test("SEAL: a non-secret field on a sealed cred passes the seal check (but still origin-checked)", () => {
  ...
  assert.doesNotThrow(() => assertFillAllowed({ ...sealedWallet, publicKey: "abc" }, "rpc.x", "publicKey"));
});

test("AUDIENCE: typing is refused on a host not in the credential's domains", () => {
  assert.throws(() => assertFillAllowed(login(), "attacker.example", "password"), /allowlist/i);
...
test("AUDIENCE: typing is allowed on a listed host and via a wildcard", () => {
  assert.doesNotThrow(() => assertFillAllowed(login(), "portail.unige.ch", "password"));
  // A wildcard covers the IdP host an SSO login redirects to.
  assert.doesNotThrow(() =>
    assertFillAllowed(login({ domains: ["*.unige.ch"] }), "adfs.unige.ch", "password"),
  );
...
test("a bare '*' domain matches nothing (deny) — consistent with hostMatchesAllowlist", () => {
  assert.throws(() => assertFillAllowed(login({ domains: ["*"] }), "portail.unige.ch", "password"), /allowlist/i);
});
