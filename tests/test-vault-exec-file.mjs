...
async function makeVaultWithSecret(dir) {
  ...
  await writeJsonFile(statePaths(dir).mainKey, {
    version: 1, agentId: "main", keyNonce: 0, createdAt: 1,
    publicKeyPem, privateKeyPem, fingerprint: fingerprintPublicKeyPem(publicKeyPem),
  });
...
test("exec --file writes a 0600 temp file, passes its path, shreds it after", async () => {
  ...
  try {
    ...
    assert.ok(!(r.stdout || "").includes("MOCKKEYLINE1"), "stdout leaked the key");
    assert.ok(!(r.stderr || "").includes("MOCKKEYLINE1"), "stderr leaked the key");
    ...
    const mode = lines[1];
    assert.ok(filePath.startsWith(os.tmpdir()) || filePath.includes("agent-id-exec-"), `path: ${filePath}`);
    assert.equal(mode.trim(), "600", "file must be mode 0600");
    assert.ok((await readFile(probe, "utf8")).includes("MOCKKEYLINE1"), "child read the key from the file");

    // After exec exits, the temp file is gone.
    assert.equal(existsSync(filePath), false, "temp key file must be shredded + removed");
  } finally {
