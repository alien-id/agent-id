...
describe("self-signed cert generation", () => {
  ...
  it("two certs have different fingerprints", () => {
    assert.notEqual(
      generateControlCert().fingerprint,
      generateControlCert().fingerprint
    );
  });
  ...
  it("encodes every serial edge case as valid DER (no illegal padding)", () => {
    const serials = {
      "leading zero + high-bit-clear (the flake)": Buffer.from([
        0x00, 0x45, 0x9a, 0xbc,
      ]),
      "leading zero + high-bit-set (0x00 required)": Buffer.from([
        0x00, 0x80, 0x11, 0x22,
      ]),
    ...
    for (const [label, serial] of Object.entries(serials)) {
      ...
      assert.equal(
        fingerprintOfCertPem(certPem),
        fingerprint,
        `fingerprint stable: ${label}`
      );
    }
  });
});
...
function pinnedRequest({
  ...
  return new Promise((resolve, reject) => {
    ...
    const req = https.request(
      ...
      (res) => {
        const peerFp = normalizeFingerprint(
          res.socket.getPeerCertificate().fingerprint256
        );
