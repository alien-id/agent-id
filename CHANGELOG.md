...
- **Control plane is no longer trusted by loopback alone.** It binds to its own
  ...
  or pair a rogue device. **The master key is never sent in cleartext:** `/approve`
  accepts only a master key sealed (ECDH-P256 + HKDF + AES-256-GCM) to the proxy's
  per-run control-plane public key — there is no plaintext path. The approver pins
