...
- **Its own idle lock, with `--no-control`.** The next request re-opens instead of answering `401 vault_locked`. With the control plane on, the request parks for an owner approval exactly as before — self-reopen never replaces a consent/approval flow.

Re-reading is also what makes a **rotated OAuth refresh token** safe to persist: `save()` re-encrypts the proxy's whole in-memory payload, so the rotation is applied to a handle freshly read from disk, never to the startup snapshot (which would erase every credential written since). The replaced handle is locked — its master key and decrypted records are zeroed — so a reopen never leaves a second decrypted copy of the vault in the heap, and the cached OAuth access/refresh tokens are dropped with it so a re-authorized credential's new refresh token wins.
